...
Kaiseki.prototype = {
    ...
    updateObjectsBatch: function( className, updates ) {
        var self = this;
        var requests = [];
        for( var idx = 0; idx < updates.length; idx += 50 ) {
            var request = new Promise(function( resolve, reject ) {
                self.updateObject(className, updates.slice(idx, idx + 49), function( err, res, body, success ) {
                    if( !err && success ) {
                        resolve(body);
                    } else {
                        reject(err || new Error("Operations failed"));
                    }
                });
            });
    ...
    deleteObjects: function( className, data ) {
        return new Promise(function( resolve, reject ) {
            var batches = [];
            for( var idx = 0; idx < body.length; idx += 50 ) {
                var request = deleteBatch(data.slice(idx, idx + 49));
            ...
            function deleteBatch( data ) {
                return new Promise(function( resolve, reject ) {
                    ...
                    for( var i = 0; i < data.length; i++ ) {
                        requests.push({
                            'method': 'DELETE',
                            'path': '/1/classes/' + className + "/" + data.objectId
                        });
                    }
                    this._jsonRequest({
                        method: 'POST',
    ...
    deleteObjectsBatch: function( className, data ) {
        var self = this;
        var requests = [];
        for( var idx = 0; idx < body.length; idx += 50 ) {
            var request = new Promise(function( resolve, reject ) {
                self.deleteObjects(className, data.slice(idx, idx + 49), function( err, res, body, success ) {
                    if( !err && success ) {
                        resolve(body);
                    } else {
                        reject(err || new Error("Operation failed"));
                    }
                });
                requests.push(request);
            });
    ...
    deleteObjects: function( className, data, callback ) {
        var requests = [];
        for( var i = 0; i < data.length; i++ ) {
            requests.push({
                'method': 'DELETE',
